...
*CASE 1* To select 3 lines, do ~moder-line~ 3 times. ~moder-line~ will create an expandable line selection.

*CASE 2* To reverse select 3 lines, do ~negative-argument~, then ~moder-line~ 3 times. ~line~ will follow the direction.

*CASE 3* To select the previous, current and next words, do ~moder-mark-word~, ~moder-back-word~ and ~moder-next-word~.
Back and next will expand, because ~moder-mark-word~ creates an *expandable word* selection.

*CASE 4* To select the next word, do ~moder-next-word~ twice.
The current word won't be selected because ~moder-next-word~ will create a *nonexpandable word* selection.
...
*FEATURE 1* For text swap. See command ~moder-swap-grab~ and ~moder-sync-grab~.

*FEATURE 2* To represent a range for kmacro application. See Moder BEACON state.

*FEATURE 3* To mark a text to be inserted in the minibuffer prompt later. See ~moder-grab-fill-commands~.
...
*** The answer from Moder

Moder embraces kmacro, and tries to improve the experience by collapsing undo boundary and introducing BEACON state.

(text-mode is used here, no additional setup required, assuming moder-setup for Qwerty is used)
...
- select the whole content, then activate a secondary selection with ~G(moder-grab)~
...
- select the whole content, then activate a secondary selection with ~G(moder-grab)~
- ~-(negative-argument) f(moder-find) -~ to search backwards for the character ~-~, which will create a fake cursor at each ~-~
- quick start recording and switch to insert state with ~c(moder-change)~ (the character under the current cursor is deleted)
- type ~_~
...
- select the whole content, then activate a secondary selection with ~G(moder-grab)~
- ~x(moder-line)~ to create fake regions at each line
- ~F3~ to start macro recording (default fake cursors are in the same column)
- select bar with ~w(mark-word)~, then activate a secondary selection with ~G(moder-grab)~
- select foo, swap with the secondary selection with ~R(moder-swap-grab)~.
- ~F4~ to end macro recording and apply to all fake cursors
...
- quick start recording and switch to insert state with ~c(moder-change)~ (current bar is deleted)
- type baz
...
Emacs is the one editor with the most modal editing schemes in the world. Before I started working on Moder, there were a few options (listed at the end).
...
God mode lacks a set of commands which is necessary for maximizing the benefits of single-key commands.
...
It's very close to what I wanted before I started with Moder.

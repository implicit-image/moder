...
Moder will find underlying commands based on keybindings. You may find abnormal behaviour if you changed
some default keybindings.  For example: forward-char on ~C-f~ is used by ~moder-right~. If you changed this keybinding,
you have to change variable ~moder--kbd-forward-char~ to the correct value.

Example:

#+BEGIN_SRC emacs-lisp
  (setq moder--kbd-forward-char "<right>")

  ;; or bind it to the function directly
  (setq moder--kbd-forward-char #'forward-char)
#+END_SRC
...
[[https://github.com/moder-edit/moder/blob/master/KEYBINDING_QWERTY.org][QWERTY]] - [[https://github.com/moder-edit/moder/blob/master/KEYBINDING_DVORAK.org][DVORAK]] - [[https://github.com/moder-edit/moder/blob/master/KEYBINDING_DVP.org][PROGRAMMER DVORAK]] - [[https://github.com/moder-edit/moder/blob/master/KEYBINDING_COLEMAK.org][COLEMAK]]

You can very easily change any of these when you setup Moder.

* Tutor inside Emacs

There is a tutor inside Emacs after installing Moder. Open it with ~M-x moder-tutor~.

* States

Moder's modal editing has 5 states: NORMAL, INSERT, MOTION, KEYPAD and BEACON.
...
You can switch to INSERT state with ~moder-insert/append/change/open-below/open-above~.

There's nothing special in INSERT state, except ~ESC~ is bound to ~moder-insert-exit~. Use it to go back to NORMAL state.

NOTE:

If you enter INSERT state by ~moder-change~, the inserted content will be selected when you exit.

** NORMAL

NORMAL state is the default state for text editing.

For commands in NORMAL state is defined via ~moder-normal-define-key~.
...
#+begin_src emacs-lisp
  (moder-motion-define-key '("j" . next-line))
#+end_src
...
This can be changed by setting ~moder-keypad-leader-dispatch~.

#+begin_src emacs-lisp
  ;; Some examples
  (setq moder-keypad-leader-dispatch "C-x") ; Dispatch to the keymap at C-x
  (setq moder-keypad-leader-dispatch ctl-x-map) ; Dispatch to the ctl-x-map
#+end_src
...
- ~moder-left/right~ will create cursors in current column.
- ~moder-next/back-word/symbol~ will create cursors at words'/symbols' beginning or end.
- ~moder-mark-word/symbol~ will create regions for every same words/symbols.
- ~moder-visit/search~ will create regions for every same regexp.
- ~moder-find/till~ will create cursors for every same characters.
- ~moder-line~ will create regions for every N lines. (N is the number of selected lines).
- ~moder-join~ will create cursors for each indentation beginning.

Once you have fake cursors/regions, you have two options:
- *quickly & simple* Switch to INSERT state and *start recording kmacro* with ~moder-insert/append/change~,

...
~moder-search~ can be used for further searching with the same condition.
...
~moder-visit-sanitize-completion~ to nil.
...
This command support fallback behaviour, default to ~moder-change-char~.
...
This command supports fallback behaviour. Default to ~moder-C-k~.

*** moder-C-k

Call the command on ~C-k~.

*** moder-C-d (moder-delete)

Call the command on ~C-d~.

*** moder-kill-word, moder-backward-kill-word, moder-kill-symbol, moder-backward-kill-symbol

Versions of ~kill-word~ and ~backward-kill-word~ that respect ~moder-word-thing~
and ~moder-symbol-thing~. May be bound to M-d and M-DEL in place of ~kill-word~
...
This command support fallback behaviour, default to ~moder-pop-grab~.
...
Additionally, grab current position(thus you can come back later with ~moder-pop-grab~).
...
This command is used as a fallback behaviour for ~moder-pop-selection~ by default.
...
The "reverse" version of  ~moder-pop-to-mark~.

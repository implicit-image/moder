...
Similar to ~moder-define-keys~.  Define key in NORMAL state.

** moder-motion-define-key

Similar to ~moder-define-keys~.  Define key in MOTION state.

** moder-leader-define-key

Similar to ~moder-define-keys~.  Define key in leader keymap.

The keymap used is the one listed in ~moder-keymap-alist~.

By default, it is ~mode-specific-map~.

** moder-setup-indicator

A helper function that puts an indicator at the beginning of mode-line. If you want customize mode-line by hand, see `moder-indicator`.
...
#+begin_src emacs-lisp
  (setq-default mode-line-format '((:eval (moder-indicator)) ...))
#+end_src

** moder-thing-register

Register a thing which can be used for ~moder-beginning/end/inner/bounds-of-thing~.  To bind it to a key, check variable ~moder-char-thing-table~.
...
#+begin_src emacs-lisp
  (setq moder-paren-keymap (make-keymap))
  (moder-define-state paren
    "moder state for interacting with smartparens"
    :lighter " [P]"
    :keymap moder-paren-keymap)

  ;; moder-define-state creates the variable
  (setq moder-cursor-type-paren 'hollow)

  (moder-define-keys 'paren
    '("<escape>" . moder-normal-mode)
    '("l" . sp-forward-sexp)
    '("h" . sp-backward-sexp)
    '("j" . sp-down-sexp)
    '("k" . sp-up-sexp)
...
Similarly to =define-minor-mode=, your last parameter to =moder-define-state= may be
a single lisp form that is run every time the internal minor mode is entered
and exited.

If you already have a minor mode that you just need to register with moder, then
see the documentation for the internal function =moder-register-state=.

* Variables

** moder-mode-state-list
...
A list of rules: each of which specifies the initial Moder state for a major mode.

Value should be a list, each item is ~(major-mode . init-state)~.
The ~init-state~ can be any state, including custom ones.

By default, Moder uses ~motion~ for special modes (in which none of
...
** moder-expand-exclude-mode-list
...
** moder-expand-hint-remove-delay

Default: 1.0

The delay before the position hint disappears.

** moder-selection-command-fallback
...
** moder-keypad-self-insert-undefined
...
Used by ~moder-beginning-of-thing~, ~moder-end-of-thing~, ~moder-inner-of-thing~ and ~moder-bounds-of-thing~.

** moder-replace-state-name-list
...
An association list of moder state symbols to indicator face symbols.

** moder-display-thing-help

Default: ~t~

Whether to display the help prompt for ~moder-inner/bounds/begin/end-of-thing~.

** moder-keypad-describe-delay

Default: ~0.5~

The delay in seconds before popup keybinding descriptions appear.

** moder-grab-fill-commands

Default: ~(moder-query-replace moder-query-replace-regexp)~

A list of commands that moder will auto fill with grabbed content.

** moder-goto-line-function
...
** moder-visit-collect-min-length

Default: ~1~

Minimal length when collecting symbols for ~moder-visit~.

** moder-visit-sanitize-completion
...
** moder-expand-selection-type

Default: ~select~

The type of selection activated by ~moder-expand-*~ commands.
...
keymap at a specified key binding.  Nil stands for taking leader keymap from ~moder-keymap-alist~.

** moder-keypad-leader-transparent
...
** moder-state-mode-alist
Association list of symbols of moder states to their corresponding mode functions.

** moder-update-cursor-functions-alist
...
should use ~moder--set-cursor-type~ and ~moder--set-cursor-color~ to update the cursor.
...
#+BEGIN_SRC emacs-lisp
(defun moder--update-cursor-custom ()
  (progn
    (moder--set-cursor-type 'hollow)
    (moder--set-cursor-color 'moder-insert-cursor)))
(add-to-list 'moder-update-cursor-functions-alist
             '((lambda () (and (moder-insert-mode-p)
                               (eq major-mode 'org-mode)))
               . moder--update-cursor-custom))
#+END_SRC
...
change the variable =moder-cursor-type-insert=.

** moder-keymap-alist

Association list of symbols to their corresponding keymaps. Used
to generate =moder-*-define-key= helpers.

** moder-word-thing, moder-symbol-thing

The things used by moder for marking/movement by words and symbols, respectively.
...
By customizing these variables, you can make Moder use your own definitions for
...
(setq moder-word-thing 'vimlike-word)
#+end_src

Moder also provides ~moder-kill-word~ and ~moder-backward-kill-word~, versions of
~kill-word~ and ~backward-kill-word~ that respect ~moder-word-thing~.

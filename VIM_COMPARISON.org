...
Despite being another modal editing scheme, moder is not very similar
...
in moder, since it's essentially built into normal mode. Almost every
...
acts as a quantifier, and =w= is the so called "verb." In moder, we think
about commands the opposite way. We'd first select a single word, with
=moder-next-word= (which, critically, both moves the point *and* creates a
...
press =c3w=. In moder, this would be =e3c= since the grammar is inverted.

Moder has the clear advantage that when you create the selection, you
...
Since many people who want to give moder a try, come from a Vim / Evil
background, here is an overview of typical Vim keybindings for common
actions and how they can be achieved in moder.

*Note:*

We provide only command names here, this is because
moder has no built-in keybind set (yet?). If you decide to use the
suggested [[https://github.com/moder-edit/moder/blob/master/KEYBINDING_QWERTY.org][QWERTY]] keybindings, you can look up the default binding to
each command there.

For ease of reading, all ~moder-~ prefixes are stripped.
...
| C-o      | moder-pop-to-mark                  | last position in jumplist; mark-ring based, only in current buffer                                |
| C-i      | moder-unpop-to-mark                | next position in jumplist; mark-ring based, only in current buffer                                |

* Search

| Vim | Moder                                    | Description                                                                             |
|-----+-----------------------------------------+-----------------------------------------------------------------------------------------|
...
|-----+-------------+-------------------------------------------------------|
| i   | insert      | insert                                                |
| a   | append      | append (needs ~(setq moder-use-cursor-position-hack t)~) |
...
| u              | undo                     | undo                                                                                                                                                              |
| C-r            | cancel undo              | redo; Emacs has a [[https://www.emacswiki.org/emacs/RedoMode][different undo system]] with a stack, so to undo the undo, you do some action which has no effect and afterwards ~moder-undo~ works in the other way |

* Macros

Vim uses registers to store macros. Moder only has a key to start a
macro and afterwards play it. When a new macro is recorded the old one
will be overridden.  The reason is that moder just wraps the [[https://www.emacswiki.org/emacs/KeyboardMacros][default
...
Except for some cases, namely =moder-word=, =moder-line= and =moder-block=,
moder generalizes the idea of selection in and around "things". You may
select inside any "thing" by first calling =moder-inner-of-thing= and
then following the onscreen prompts. Moder makes it easy to define your
...
Generally there is no mode in moder similar to command mode. But there
